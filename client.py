...
def mousePressed(event, data):
    msg = "newRectangle %d %d\n" % (event.x, event.y)
    data.me.newRectangle(event.x, event.y)
...
def timerFired(data):
    ...
    while (serverMsg.qsize() > 0):
      ...
      try:
        ...
        elif(command == "newRectangle"):
          PID = msg[1]
          x = int(msg[2])
          y = int(msg[3])
          data.otherStrangers[PID].newRectangle(x, y)
      except:
        print("failed")
      serverMsg.task_done()
...
def redrawAll(canvas, data):
    ...
    for playerName in data.otherStrangers:
      ...
      data.otherStrangers[playerName].drawRectangles(canvas)
    # draw me
    data.me.drawDot(canvas, "red")
    data.me.drawRectangles(canvas)
